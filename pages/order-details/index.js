...
Page({
    ...
    onShow : function () {
      ...
      wx.request({
        ...
        success: (res) => {
          ...
          // 获取快递信息
          let order_num = res.data.result.orderNum
          console.log(order_num)
          wx.showLoading({
            title: '请稍等'
          })
          wx: wx.request({
            url: api.data.get_kuaididanhao,
            ...
            success: function (res) {
              ...
              if(res.data.state == 200) {
                that.setData({
                  wuliu_data: res.data.shippingNum,
                  wuliu_status: 200
                })
                // if (res.data.jsonArr[0].state == 200) {
                //   that.setData({
                //     wuliu_data: res.data.shippingNum,
                //     wuliu_status: 200
                //   })
    ...
    goto: function(e) {
      ...
      // console.log(orderId)
      wx:wx.navigateTo({
        url: '../od_change_site/od_change_site?id=' + orderId
      })
    },
      /**
     * 用户点击右上角分享
